...
class QvxFileRowIterator extends CloseableRowIterator {
    ...
    private final QvxFileReaderNodeSettings m_frSettings;
    ...
    private final QvxDataCellFactory m_cellFactory;
    ...
    QvxFileRowIterator(final QvxFileReaderNodeSettings frSettings,
        ...
        m_cellFactory = new QvxDataCellFactory();
        ...
        if (frSettings.getFileHasRowHeaders()) {
            ...
            if (frSettings.getRowHeaderPrefix() != null) {
                ...
                m_rowHeaderPrefix = QvxFileReaderNodeSettings.DEF_ROWPREFIX;
            }
        }
    ...
    public DataRow next() {
        ...
        for(DataCell datacell:row) {
        	DataType type = m_tableSpec.getColumnSpec(index).getType();
        	
        	if(type.equals(IntCell.TYPE)) {
        			System.out.println("[ " + index + "] = " + m_tableSpec.getColumnSpec(index).getName());
        			row[index++] = m_cellFactory.createDataCellOfType(IntCell.TYPE, "1");
        	}
        	else
            	
            if(type.equals(DoubleCell.TYPE)) {
        			System.out.println("[ " + index + "] = " + m_tableSpec.getColumnSpec(index).getName());
        			row[index++] = m_cellFactory.createDataCellOfType(DoubleCell.TYPE, "1.0");
        	}
        	else
            if(type.equals(LongCell.TYPE)) {
        			System.out.println("[ " + index + "] = " + m_tableSpec.getColumnSpec(index).getName());
        			row[index++] = m_cellFactory.createDataCellOfType(LongCell.TYPE, "100");
        	}
        	else
             	if(type.equals(BooleanCell.TYPE)) {
        			System.out.println("[ " + index + "] = " + m_tableSpec.getColumnSpec(index).getName());
        			row[index++] = m_cellFactory.createDataCellOfType(BooleanCell.TYPE, "true");
        	}
        	else
             	if(type.equals(TimestampCell.TYPE)) {
        			System.out.println("[ " + index + "] = " + m_tableSpec.getColumnSpec(index).getName());
        			row[index++] = m_cellFactory.createDataCellOfType(TimestampCell.TYPE, "20190101");
        	}
        	else
                   	
         	if(type.equals(StringCell.TYPE)) {
         			System.out.println("[ " + index + "] = " + m_tableSpec.getColumnSpec(index).getName());
        			row[index++] = m_cellFactory.createDataCellOfType(StringCell.TYPE, "testData");
        	}
    ...
    private DataCell createNewDataCellOfType(final DataType type,
        ...
        DataCell result = m_cellFactory.createDataCellOfType(StringCell.TYPE, "testData");
     ...
     */
    private QvxFileReaderException prepareForException(final String msg,
        ...
        return new QvxFileReaderException(msg, errRow, lineNumber);
